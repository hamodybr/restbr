...
(() => {
  ...
  function boot(){
    forceInternalPlan(); loadOnboardingPreflight();
    const add = $('#addRestaurantBtn'); if(add) add.addEventListener('click', () => setTimeout(forceInternalPlan, 0));
    const form = $('#restaurantForm'); if(form) form.addEventListener('submit', forceInternalPlan, true);
    const list = $('#restaurantList'); if(list){ cleanPlanUi(); new MutationObserver(cleanPlanUi).observe(list,{childList:true,subtree:true}); }
