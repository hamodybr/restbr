...
(() => {
  ...
  function normalizeSlug(value) {
    return String(value || '').toLowerCase().trim().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '').replace(/-+/g, '-').replace(/^-|-$/g, '').slice(0, 63);
  }
  ...
  function friendlyErrors(errors) {
    const labels = {
      restaurant_name_required: 'اسم المطعم مطلوب.', invalid_slug: 'الـSlug غير صالح. استخدم أحرف إنجليزية صغيرة وأرقام وشرطة فقط.', reserved_slug: 'هذا الـSlug محجوز للمنصة. اختر اسمًا آخر.', invalid_default_language: 'اللغة الافتراضية غير صالحة.', invalid_plan: 'الخطة غير صالحة.', slug_already_exists: 'هذا الـSlug مستخدم مسبقًا.', hostname_already_exists: 'هذا الدومين مستخدم مسبقًا.', restaurant_not_draft: 'المطعم ليس بحالة Draft.', settings_missing: 'إعدادات المطعم ناقصة.', primary_domain_missing: 'الدومين الرئيسي ناقص.', hostname_mismatch: 'الدومين لا يطابق الـSlug.', domain_kind_invalid: 'نوع الدومين غير صحيح.', domain_not_pending: 'الدومين ليس بحالة Pending.', subscription_missing: 'الاشتراك ناقص.', subscription_inactive: 'الاشتراك غير فعال.', router_origin_mismatch: 'Router origin غير صحيح.', router_base_path_mismatch: 'Router base path غير صحيح.'
    };
  ...
  async function client() {
    ...
    if (!response.ok || !config?.ok || !config?.supabase_url || !config?.publishable_key) throw new Error('تعذر تحميل إعدادات RESTBR Platform.');
    restbrClient = window.supabase.createClient(config.supabase_url, config.publishable_key, { auth: { persistSession: true, autoRefreshToken: true, detectSessionInUrl: false } });
    return restbrClient;
  }
  ...
  async function verifyProvision(restaurantId) { const status = await rpc('admin_restaurant_provision_status', { p_restaurant_id: restaurantId }); if (!status?.ok) throw new Error('تعذر التحقق من تهيئة المطعم.'); return status; }
  async function activateProvision(restaurantId) { return rpc('admin_activate_restaurant', { p_restaurant_id: restaurantId }); }
  ...
  async function runOnboarding(event) {
    ...
    if (!params.p_name || params.p_slug.length < 2) { setMessage('اكتب اسم المطعم وSlug صالح مثل yourcoffee.', 'error'); return; }
    if (button) { button.disabled = true; button.textContent = 'جاري فحص الجاهزية...'; }
    try {
      ...
      if (!verified.ready) { setMessage(`تم إنشاء المطعم كـ Draft فقط ولن يظهر للعامة. فشل Verify: ${friendlyErrors(verified.errors) || 'جاهزية غير مكتملة.'}`, 'error'); setTimeout(() => location.reload(), 2200); return; }
      const approvedActivation = window.confirm(`Verify ناجح ✓\n\nSettings: موجودة\nDomain: ${verified?.domain?.hostname || hostname} — Pending\nSubscription: ${verified?.subscription?.status || 'active'}\nMenu seed: ${verified?.menu_seed?.categories || 0} أقسام / ${verified?.menu_seed?.products || 0} منتجات\n\nالآن فقط يمكن تفعيل المطعم للعامة.\n\nهل تريد Activate؟`);
      if (!approvedActivation) { setMessage(`تم حفظ ${hostname} كـ Draft آمن. لم يتم تفعيله للعامة.`, 'success'); setTimeout(() => location.reload(), 1800); return; }
      ...
      if (button) { button.disabled = false; button.textContent = 'إنشاء المطعم'; }
    }
  }
  ...
  async function handleDraftActivation(event) {
    ...
    try {
      const verified = await verifyProvision(restaurantId);
      if (!verified.ready) { window.alert(`لا يمكن تفعيل Draft لأن Verify فشل:\n${friendlyErrors(verified.errors) || 'جاهزية غير مكتملة.'}`); return; }
